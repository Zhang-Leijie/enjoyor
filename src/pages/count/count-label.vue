<template>
	<div>
		<el-breadcrumb separator="/">
		  	...
		  	<el-breadcrumb-item>项目标签统计</el-breadcrumb-item>
		</el-breadcrumb>
		<div class="inlist-search clearfix">
			...
			<!-- <el-select placeholder="项目标签" class="searinfo2">
				<el-option
			      label="全部"
			      :value="10000">
			    </el-option>
			...
			<div class="button blue" style="float:left;margin-left:5px;" @click="getInfo">
				查询
			</div>
			<div class="button blue" style="float:left;margin-left:5px;" @click="reset">
				重置
			</div>
		</div>
		<div  class="ratechart2" id='ratechart1'></div>
		<div  class="ratechart2" id='ratechart2'></div>
		<div  class="ratechart2" id='ratechart3'></div>
	</div>
</template>
<script>
	// 引入 ECharts 主模块
	...
	import { vocation_project,getSearchList } from '../../ajax/get.js'
	export default {
	    data() {
	        return {
	        	fond:null,
	    ...
	    methods: {
	    	...
	    	getInfo(){
	    		...
	    		vocation_project({
                    ...
                    res.data0.forEach(function(list){
                    	self.onelabel.push({value:list[0],name:list[1]})
                    })  
                    res.data1.forEach(function(list){
                    	self.twolabel.push({value:list[0],name:list[1]})
                    })
                    res.data2.forEach(function(list){
                    	self.threelabel.push({value:list[0],name:list[1]})
                    })
                    this.getDateone()
                    this.getDatetwo()
                    this.getDatethree()            
	        ...
	        getDatethree(){
	        	var myChart = echarts.init(document.getElementById('ratechart3'))
	        	myChart.setOption({
				    title : {
				        text: '三级标签与项目数量',
				        // subtext: '纯属虚构',
